...
const FILENAME = "MidiKeyParticlesR2";
...
const MidiKeyParticlesR2 = (props: Props) => {
  const Sketch = (p: p5) => {
    let time = 0;
    ...
    p.setup = () => {
      ...
      p.frameRate(60);
      p.camera(0, 300, p.height / 2 / p.tan(p.PI / 6), 250, 500, 0, 0, 1, 0);
    };
    p.keyPressed = () => {
    ...
    p.draw = () => {
      ...
      drawNotes.forEach((note) => {
        // We generate a z-index offset (or they all stack and clash)
        // The offset is per-key stored in a hash map `zOffset`
        // And we increment once per unique key, which we keep track using a cache
        if (!(note.note in zOffset)) zOffset[note.note] = 0;
        const noteId = `${note.note}-${note.created}`;
        if (!zOffsetCache.includes(noteId)) {
          zOffset[note.note] += 10;
          zOffsetCache.push(noteId);
        }
        ...
        p.translate(containerX, y, zOffset[note.note]);
        p.rect(0, 0, containerWidth, NOTE_HEIGHT, NOTE_BORDER_RADIUS);
        p.strokeWeight(0);
...
export default MidiKeyParticlesR2;
