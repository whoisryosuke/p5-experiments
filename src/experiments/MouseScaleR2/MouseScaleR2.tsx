...
const MouseScaleR2 = ({ width, height, ...props }: Props) => {
    ...
    p.draw = () => {
      ...
      [...new Array(COUNT)].forEach((_, xIndex) => {
        [...new Array(COUNT)].forEach((_, yIndex) => {
          ...
          const scale = p.map(
            // Proper distance formula - gives a circular effect instead of diamond shaped
            Math.sqrt((x - p.mouseX) ** 2 + (y - p.mouseY) ** 2) / 2,
            0,
            p.windowWidth,
            0,
            10
