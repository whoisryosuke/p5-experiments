...
const WaveformBallR3 = (props: Props) => {
  const Sketch = (p: p5) => {
    const skipChunk = 4;
    let y = 100;
    ...
    const renderParticle = (particle: WaveformParticle) => {
      ...
      // p.text("💧", x, y);
      p.vertex(animation.x, animation.y, animation.z);
      p.pop();
    };
    ...
    p.draw = () => {
      ...
      p.beginShape();
      particles
        .filter((particle) => particle.chunk == 0)
        .forEach(renderParticle);
      p.endShape();
      p.beginShape();
      particles
        .filter((particle) => particle.chunk == 1)
        .forEach(renderParticle);
      p.endShape();
      p.beginShape();
      particles
        .filter((particle) => particle.chunk == 2)
        .forEach(renderParticle);
      p.endShape();
    };
  };
