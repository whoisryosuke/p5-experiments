...
type Particle = {
  // The note index in the current MIDI track
  note: number;
  animation: {
    opacity: number;
  };
...
const MidiCirclePlayerR8 = (props: Props) => {
  const Sketch = (p: p5) => {
    ...
    p.setup = () => {
      ...
      // Debug
      // setInterval(() => {
      //   particles?.[0]?.forEach((particle, index) => {
      //     if (particle.animation.opacity < 1)
    ...
    const deleteParticle = (trackIndex: number, particleIndex: number) => {
      // console.log("[DELETE PARTICLE]", trackIndex, particleIndex);
      particles[trackIndex] = particles[trackIndex].filter(
        (particle) => particle.note == particleIndex
      );
    };
    ...
    p.draw = () => {
      ...
      const durationInSeconds = TIMELINE_DURATION;
