...
const FILENAME = "MidiKeyParticlesR4";
...
const MidiKeyParticlesR4 = (props: Props) => {
  const Sketch = (p: p5) => {
    let time = 0;
    ...
    p.draw = () => {
      ...
      drawNotes.forEach((note) => {
        ...
        const fadeUpAnimation = {
          ...
          end: { y: startY - 1000, opacity: 1 },
          animate: Math.min((time - note.created) / NOTE_DURATION, 1),
        };
        const fadeDownAnimation = {
          start: { y: startY - 1000, opacity: 1 },
          end: { y: startY - 2000, opacity: 0 },
          animate: Math.min(
        ...
        p.translate(containerX - 800, y, 0);
...
export default MidiKeyParticlesR4;
