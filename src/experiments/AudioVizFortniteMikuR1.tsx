...
const AudioVizFortniteMikuR1 = (props: Props) => {
  const Sketch = (p: p5) => {
    let y = 100;
    ...
    p.draw = () => {
      ...
      for (let i = 0; i < bufferLength / skipChunk; i++) {
        if (i === 0) console.log(dataArray[i + skipChunk]);
        ...
        color.setAlpha(255 / 4);
        p.stroke(color);
        p.strokeWeight(4 * textHeight);
        // p.text("💧", x, y);
        // p.point(positionExploded.x, positionExploded.y, v * 100);
        p.translate(x, y, (v * 100) / 2);
        p.emissiveMaterial(0, 50, 255);
        p.box(10, 10, v * 100);
        p.pop();
      }
      // p.endShape();

      playbackButton = p.createButton("Play/Pause");
      playbackButton.position(0, 0);
      playbackButton.mousePressed(handlePlaybackButton);
    };
  };
...
export default AudioVizFortniteMikuR1;
