...
const FILENAME = "WaterfallWaveformR2";
...
const WaterfallWaveformR2 = (props: Props) => {
  ...
  const Sketch = (p: p5) => {
    ...
    let dataBank: Array<Float32Array<ArrayBuffer>>;
    ...
    p.setup = () => {
      ...
      // Setup bank
      dataBank = new Array(WAVE_COUNT).fill(new Float32Array(bufferLength));
    ...
    p.draw = () => {
      ...
      p.noFill();
      p.strokeWeight(2);
      const strokeColor = p.color(BASE_COLORS[`blue-4`]);
      p.stroke(strokeColor);
      strokeColor.setAlpha(255 * 0.25);

      p.box(audioWidth, 500, 500);
      p.translate(-audioWidth / 2, 225, 250);

      for (let waveIndex = WAVE_COUNT - 1; waveIndex >= 0; waveIndex--) {
        const waveData = dataBank[waveIndex];
        p.beginShape();
        p.translate(0, 0, waveIndex * -10);
        const wavePercent = waveIndex / WAVE_COUNT;
        strokeColor.setAlpha(255 * wavePercent);

        for (let i = 0; i < bufferLength; i++) {
          // All the magic happens in the y axis here
          // The audio data comes back in a range from around -10db to -100db
          const v = p.map(waveData[i], -10, -150, 0, 1, false);
          const clampedV = p.constrain(v, -100, 1);
          const amplifiedV = Math.pow(v, 1.4);
          // This centers line on screen
          const y = (clampedV * p.height) / 2 - p.height / 2;
...
export default WaterfallWaveformR2;
