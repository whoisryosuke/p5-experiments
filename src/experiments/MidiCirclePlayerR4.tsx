...
const FILENAME = "MidiCirclePlayerR4";

const MidiCirclePlayerR4 = (props: Props) => {
  const Sketch = (p: p5) => {
    let time = 0;
    let midiFile: Midi;
    ...
    p.draw = () => {
      ...
      p.textStyle(p.BOLD);
      const TRACK_LABEL_FONT_SIZE = 14;
      ...
      noteTracks.forEach((track, index) => {
        ...
        p.text(
          trackLabel,
          x,
          y - height / 2 + (STROKE_SIZE * 1.1 * index + 1) + 5
        );
      });
...
export default MidiCirclePlayerR4;
