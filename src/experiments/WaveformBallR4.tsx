...
const WaveformBallR4 = (props: Props) => {
  const Sketch = (p: p5) => {
    const skipChunk = 4;
    const repeatParticles = 4;
    let y = 100;
    ...
    p.setup = () => {
      ...
      particles = new Array(bufferLength * repeatParticles)
        .fill(0)
        .map((_, id) => ({
          id: id % bufferLength,
          chunk: (id % bufferLength) % skipChunk,
          animation: {
            x: 0,
            y: 0,
            z: 0,
          ...
          // Every next set of particles is slower
          speed: Math.round(id / bufferLength) + 1,
        }));
    ...
    p.draw = () => {
      ...
      if (p.frameCount % 30 != 0) {
        ...
        for (let i = 0; i < bufferLength * repeatParticles; i++) {
          const dataId = i % bufferLength;
          ...
          animate(
            particles[particleId].animation,
            {
              ...
              z: v * 100 * particles[particleId].speed,
              opacity: textHeight - particles[particleId].speed / 4,
            },
            {
              duration: 0.1 * particles[particleId].speed,
              autoplay: true,
            }
          );
        }
