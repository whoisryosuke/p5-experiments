...
const WaveformSamplerR2 = (props: Props) => {
  const Sketch = (p: p5) => {
    ...
    let waveforms: Uint8Array<ArrayBuffer>[] = [];
    let lastCheck = 0;
    ...
    p.draw = () => {
      ...
      waveforms.forEach((waveform, waveformIndex) => {
        ...
        for (let i = 0; i < bufferLength; i++) {
          ...
          const v = p.map(waveform[i], 0, audioFreqencyHeight, 0, 1);
          // This centers line on screen
          const y = (v * p.height) / 2 - p.height / 2;
          const x = i * segmentWidth;

          p.vertex(x, y, waveformIndex * 10);
        }
        p.endShape();
      });
...
export default WaveformSamplerR2;
